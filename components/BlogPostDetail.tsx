    ...
    if (!post) {
        return (
            <div className="flex flex-col items-center justify-center h-full p-8">
                <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4">{t('blog.postNotFound')}</h2>
                <button onClick={onBack} className="px-6 py-3 bg-teal-600 text-white rounded-xl font-medium hover:bg-teal-700 transition-colors">
                    {t('blog.backToBlog')}
                </button>
            </div>
        );
        ...
        <div className="flex flex-col h-full bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 overflow-hidden">
            ...
            >
                ...
                <article className="max-w-5xl mx-auto px-4 sm:px-6 py-8 md:py-10 w-full">
                    <button
                        onClick={onBack}
                        className="inline-flex items-center gap-2 px-4 py-2 mb-6 text-teal-600 dark:text-teal-400 hover:text-teal-700 font-semibold transition-all bg-teal-50 dark:bg-teal-900/20 hover:bg-teal-100 rounded-xl group"
                    >
                    ...
                    <header className="mb-8">
                        ...
                        <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-gray-900 dark:text-gray-100 mb-5 leading-tight">
                        ...
                        {/* Author + Share row */}
                        <div className="flex items-center justify-between flex-wrap gap-3 pb-6 border-b-2 border-gray-200 dark:border-gray-700">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="font-bold text-gray-900 dark:text-gray-100">{post.author}</p>
                                    <p className="text-xs text-gray-500 dark:text-gray-400">{t('blog.author')}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={shareFacebook} aria-label="Share on Facebook"
                                    className="w-9 h-9 flex items-center justify-center rounded-xl bg-blue-600 hover:bg-blue-700 text-white transition-colors shadow-sm">
                                ...
                                <button onClick={handleCopy} aria-label="Copy link"
                                    className="flex items-center gap-1.5 px-3 py-2 bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-700 dark:text-gray-300 rounded-xl text-xs font-semibold transition-colors">
                    ...
                    {/* Table of Contents */}
                    {toc.length > 2 && (
                        <div className="mb-8 bg-teal-50 dark:bg-slate-800 border border-teal-200 dark:border-teal-800 rounded-2xl overflow-hidden">
                            <button
                            ...
                            >
                                ...
                                <span className="text-teal-600 dark:text-teal-400 text-xs">{showToc ? '▲' : '▼'}</span>
                            </button>
                            {showToc && (
                    ...
                    <div className="mt-8 p-5 bg-gray-50 dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-gray-700">
                        <p className="text-sm font-bold text-gray-700 dark:text-gray-300 mb-3">{t('blog.sharePost')}</p>
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <button onClick={shareFacebook} className="flex items-center gap-1.5 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-xl transition-colors">
                            ...
                            <button onClick={handleCopy} className="flex items-center gap-1.5 px-3 py-2 bg-gray-200 dark:bg-slate-700 hover:bg-gray-300 dark:hover:bg-slate-600 text-gray-700 dark:text-gray-200 text-xs font-semibold rounded-xl transition-colors">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                {relatedPosts.map(rp => (
                                    <div
                                        ...
                                        className="group bg-white dark:bg-slate-800 rounded-xl overflow-hidden cursor-pointer hover:shadow-xl transition-all border border-gray-200 dark:border-gray-700 hover:-translate-y-1 flex flex-col"
                                    >
                                        <div className="h-32 overflow-hidden bg-gray-100 dark:bg-slate-700">
                                            <img
                                                src={rp.coverImage}
                                        ...
                                        <div className="p-3 flex-1">
                                            <span className="text-xs text-teal-600 dark:text-teal-400 font-semibold">{rp.category}</span>
                                            <h4 className="text-sm font-bold text-gray-900 dark:text-gray-100 mt-1 group-hover:text-teal-600 dark:group-hover:text-teal-400 transition-colors line-clamp-2">
                                                {language === 'bn' ? rp.bnTitle : rp.title}
                                            </h4>
                                            <p className="text-xs text-gray-500 mt-1">{rp.readTime}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="mt-12 bg-gradient-to-br from-teal-500 via-cyan-500 to-blue-600 rounded-3xl p-8 text-center shadow-2xl">
                        ...
                        <button
                            onClick={onGoHome}
                            className="px-8 py-3 bg-white text-teal-600 rounded-xl font-black text-base hover:bg-gray-50 transition-all shadow-xl hover:shadow-2xl transform hover:-translate-y-0.5"
                        >
