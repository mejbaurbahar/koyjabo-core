...
import { getTrainPhotos, TrainPhoto } from '../services/communityDataService';

interface TrainImageViewerProps {
    trainId: string;
    trainName: string;
    trainBnName?: string;
    isCompact?: boolean;
...
export const TrainImageViewer: React.FC<TrainImageViewerProps> = ({ trainId, trainName, trainBnName, isCompact = false }) => {
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [imageError, setImageError] = useState(false);
    const [communityImages, setCommunityImages] = useState<TrainPhoto[]>([]);
    const [currentImageIndex, setCurrentImageIndex] = useState(0);
    const { t } = useLanguage();

    const currentCommunityImage = communityImages[currentImageIndex]?.dataUrl ?? null;
    const displayImage = currentCommunityImage || '/default-bus.svg'; // Using default-bus.svg as a generic transport placeholder if no train specific one exists

    // Reset image error when modal closes or train changes
    useEffect(() => {
        if (!isModalOpen) {
            setImageError(false);
        }
    ...
    useEffect(() => {
        setImageError(false);
    }, [trainName, trainBnName]);

    useEffect(() => {
        const loadCommunityImage = async () => {
            try {
                const photos = await getTrainPhotos(trainId);
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-500 to-indigo-600">
                            <div>
                                ...
                                {trainBnName && (
                                    <p className="text-sm text-blue-100">{trainBnName}</p>
                                )}
                            </div>
                            <button
                                onClick={handleCloseModal}
...
export default TrainImageViewer;
