    ...
    <div className="flex flex-col flex-1 min-h-0 bg-kj-bg overflow-hidden">
      <div className="flex items-center gap-3 p-4 bg-kj-panel border-b border-kj-line shrink-0">
        ...
        <div className="flex-1">
          <h1 className="text-lg font-bold text-kj-text">{lbl('Trip Reminders', 'যাত্রা রিমাইন্ডার')}</h1>
          <p className="text-xs text-kj-text-dim">{syncing ? lbl('Saving...', 'সংরক্ষণ হচ্ছে...') : `${reminders.length} ${lbl('reminders', 'রিমাইন্ডার')}`}</p>
        </div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto overscroll-y-contain touch-pan-y p-4 space-y-3 pb-nav-safe" style={{ WebkitOverflowScrolling: 'touch' }}>
              ...
              <div className="flex gap-1.5 flex-wrap">
                {DAY_LABELS.map((day, i) => (
                  <button key={i} type="button" onClick={() => toggleDay(i)}
                    className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${form.days.includes(i) ? 'bg-violet-500 text-white' : 'bg-kj-chip-bg text-kj-text-dim'}`}>
                    {day}
                  </button>
                ))}
              </div>
        ...
        {reminders.length === 0 && !showForm && (
          <div className="text-center py-10 px-4">
            ...
            <p className="text-kj-text-dim font-bold text-lg mb-1">{lbl('No reminders yet', 'এখনো কোনো রিমাইন্ডার নেই')}</p>
            <p className="text-sm text-kj-text-faint mb-6">{lbl('Set alerts for your daily commute so you never miss a bus or train.', 'নিয়মিত যাত্রার জন্য সতর্কতা সেট করুন — বাস বা ট্রেন মিস হবে না।')}</p>
            <button
            ...
            <p className="text-xs text-kj-text-faint mt-4">{lbl('Tap the + button above to add more reminders', 'আরও রিমাইন্ডার যোগ করতে উপরের + বোতামে চাপুন')}</p>
        ...
        {reminders.map(r => (
          <div key={r.id} className={`bg-kj-panel rounded-2xl p-4 border transition-all shadow-sm ${r.enabled ? 'border-violet-200 dark:border-violet-800' : 'border-kj-line opacity-60'}`}>
            <div className="flex items-start justify-between gap-2">
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <Bell className={`w-4 h-4 shrink-0 ${r.enabled ? 'text-violet-500' : 'text-kj-text-faint'}`} />
                  <h3 className="font-bold text-kj-text text-sm">{r.label}</h3>
                </div>
                <div className="flex items-center gap-3 mt-1.5 text-xs text-kj-text-dim">
                  <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {r.time}</span>
                  {r.busName && <span>🚌 {r.busName}</span>}
                </div>
                {(r.fromStop || r.toStop) && (
                  <p className="text-xs text-kj-text-faint mt-0.5">{r.fromStop} → {r.toStop}</p>
                )}
                <div className="flex gap-1 mt-2 flex-wrap">
                  {DAY_LABELS.map((d, i) => (
                    <span key={i} className={`text-xs px-1.5 py-0.5 rounded font-medium ${r.days.includes(i) ? 'bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-400' : 'bg-kj-chip-bg text-kj-text-faint'}`}>{d}</span>
                  ))}
                </div>
                <p className="text-xs text-kj-text-faint mt-1">{r.minutesBefore} {lbl('min early alert', 'মিনিট আগে সতর্ক করবে')}</p>
              </div>
