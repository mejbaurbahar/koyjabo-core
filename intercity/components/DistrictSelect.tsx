  ...
  return (
    <div className="relative group" ref={dropdownRef}>
      ...
      <div className="relative">
        ...
        {isOpen && (
          <div className="absolute z-[999] w-full mt-1 bg-kj-panel border border-kj-line dark:border-slate-600 rounded-xl shadow-lg max-h-[300px] md:max-h-[350px] overflow-hidden flex flex-col">
            {/* Search Input In Dropdown */}
            <div className="p-2 border-b border-kj-line bg-kj-panel sticky top-0">
              <input
                ref={inputRef}
                type="text"
                value={searchTerm}
                ...
                className="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-slate-700 border border-kj-line dark:border-slate-600 rounded-lg outline-none focus:ring-2 focus:ring-blue-500 text-kj-text-dim dark:text-gray-100"
              />
            </div>
            ...
            <div className="overflow-y-auto flex-1 h-full max-h-[250px] md:max-h-[300px]">
              <div
                ...
                className="px-3 py-2 text-sm md:text-base hover:bg-kj-chip-bg cursor-pointer text-kj-text-dim border-b border-gray-50 dark:border-kj-line"
              >
                {placeholder || t('common.select')}
              </div>

              {Object.keys(filteredData).length === 0 ? (
                <div className="p-4 text-center text-sm text-kj-text-faint">{t('home.noResults')}</div>
              ) : (
                Object.entries(filteredData).map(([category, locations]) => (
                  <div key={category}>
                    <div className="px-3 py-1 text-[10px] font-black uppercase tracking-widest text-red-600 dark:text-red-400 bg-gray-50 dark:bg-slate-700 sticky top-0 border-y border-kj-line dark:border-slate-600 z-10">
                    ...
                    {locations.map((location) => (
                      <div
                        ...
                        className={`px-3 py-2 text-sm md:text-base hover:bg-blue-50 dark:hover:bg-slate-700 cursor-pointer ${value === location ? 'bg-blue-50 dark:bg-slate-700 text-blue-600 dark:text-blue-400 font-medium' : 'text-kj-text-dim dark:text-slate-200'
                          }`}
                      >
                        {location}
                      </div>
